...
st.caption("Portfolio-level benchmark view for SME strategy and business development.")

if filtered_institutions.empty or filtered_scores.empty:
    st.warning("No institutions match the selected filters.")
    st.stop()

turkish_bank_types = ["Turkish Bank", "Public Bank", "Participation Bank"]
payment_fintech_types = ["Payment Kurum", "Global Fintech"]
...
k1.metric("Kurumlar tracked", len(filtered_institutions))
k2.metric(
    "Türk bankaları",
...
st.plotly_chart(
    bar_scores(
        ...
        "institution_name",
        "overall_score",
        "Overall Average Score by Kurum",
    ),
    use_container_width=True,
)

st.subheader("Kurum x Dimension Heatmap")
...
st.plotly_chart(heatmap(pivot.round(1), "Benchmark Dimension Scores"), use_container_width=True)

st.subheader("Yönetici İçgörüsü")
st.write(strategic_gap_text(filtered_scores, filtered_institutions))
st.write(
    "For V1, the main strategic signal is the convergence of SME deposits, merchant acquiring "
    "and embedded workflow distribution. Traditional banks still carry deposit and relationship "
    "depth, while global fintech references show how SME balances can be captured inside daily "
